...
ssize_t ledkeydev_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	{
		//poll사용했다면..
		//poll에서 sw_no> 0일 때 실행시켰으므로 이 코드는 의미가 없다.
		if(sw_no==0)
		{
...
static unsigned int ledkeydev_poll(struct file *filp, struct poll_table_struct *wait)
{
	...
	if(wait->_key & POLLIN)
	{
		//원래는 read함수에서 읽을 값이 없으면 프로세스를 재웠음.
		//이제 read를 먼저 호출하지 않아서 read에서 잠드는게 아님
		//select() or poll()에서 잠들어야한다.
		poll_wait(filp,&WaitQueue_Read,wait); 
		//wait에 app에 있는 timeout 값이 넘어옴.. interrupt 발생하면 깨워줌
		//안깨우면 2초 마다 buffer을 읽어옴
	}
	if(sw_no >0)//스위치가 눌렸다면
	{
		mask = POLLIN;
	}
	return mask;
