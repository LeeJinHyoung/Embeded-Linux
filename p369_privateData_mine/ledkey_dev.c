...
//static char sw_no = 0; //전역변수라 데이터 보존이 어렵다
...
int ledkeydev_open (struct inode *inode, struct file *filp)
{
	...
	*pSw_no = 0;
	filp->private_data =(void *)pSw_no;//크기 정보가 없는 void *	
	irq_init(filp);
...
//	char kbuf;
	int ret;
	char* pSw_no = (char *)filp->private_data; //크기정보가 생김
#if DEBUG
...
irqreturn_t sw_isr(int irq, void *private_data) //커널에 호출해줌..그래서 매개변수 갖고있음
{
	int i; 
