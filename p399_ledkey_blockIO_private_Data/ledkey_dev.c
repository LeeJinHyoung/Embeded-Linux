...
//		sw_irq[i] = gpio_to_irq(key[i]);
		pIsr_Info->sw_irq[i] = gpio_to_irq(key[i]);
		if(ret<0){
			printk("#### FAILED Request gpio %d. error : %d \n", key[i], ret);
			break;
		} 
...
static void ledkey_free(struct file *filp)
{
	...
	for (i = 0; i < ARRAY_SIZE(led); i++){
  	...
  	for (i = 0; i < ARRAY_SIZE(key); i++){
//		gpio_free(key[i]);
		free_irq(pIsr_Info->sw_irq[i],filp->private_data);
	}
}
...
//	filp->private_data = (void *)pSw_no;
	//보이드 포인터는 모든 포인터를 저장할 수 있지만 사용할 땐 형변
...
//	ret=copy_to_user(buf,&sw_no,count);
	...
	{
		if(pIsrInfo->sw_no==0)//키가 한 번도 안눌렸다면
		{
...
//		result = request_irq(sw_irq[i],sw_isr,IRQF_TRIGGER_RISING,sw_name[i],filp->private_data);
		result = request_irq(pSw_irq->sw_irq[i],sw_isr,IRQF_TRIGGER_RISING,sw_name[i],filp->private_data);//8키 모두 다 같은 irq핸들러인 sw_isr을 사용중.
		if(result)
		{
			printk("#### FAILED Request irq %d. error : %d \n", pSw_irq->sw_irq[i], result);
			break;
		}
	}
	return result;
